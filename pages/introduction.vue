  ...
  <section class="h-full px-9 pb-8 overflow-hidden" :class="{'bg-spotify-green': step === 0}">
    <transition
      ...
      <div v-else-if="step === 1" class="mx-2 relative text-center grid place-content-center h-full">
        ...
        <button @click="nextStep" class="bg-spotify-green absolute bottom-0 w-full py-4 rounded-full font-bold text-base inline-flex items-center justify-center gap-[0.875rem]">
          <span>Continue</span>
          <ArrowRight class="w-4 aspect-square"/>
        </button>
      </div>
      <div v-else-if="step === 2" class="mx-2 relative text-center grid place-content-center h-full">
        ...
        <div class="absolute bottom-0 w-full flex items-center gap-4">
          ...
          <button @click="routeToExploration" class="bg-spotify-green w-full py-4 rounded-full font-bold text-base inline-flex items-center justify-center gap-[0.875rem]">
            Explore your taste
          </button>
        </div>
      </div>
...
const routeToExploration = () => {
  //route to wherever the screen should route to. :)
}
</script>
